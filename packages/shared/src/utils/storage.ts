...
export async function readJsonFile<T>(filePath: string): Promise<T> {
  ...
  if (!existsSync(filePath)) {
    throw new StorageError(`File not found: ${filePath}`, 'FILE_NOT_FOUND');
  }
  ...
  try {
    ...
    throw new StorageError(`Failed to read file: ${filePath}`, 'PERMISSION_ERROR', err);
  }
  ...
  try {
    ...
    throw new StorageError(`Invalid JSON in file: ${filePath}`, 'PARSE_ERROR', err);
  }
}
 ...
 */
export async function safeReadJsonFile<T>(filePath: string): Promise<StorageResult<T>> {
  try {
...
export async function writeJsonFile<T>(filePath: string, data: T): Promise<void> {
  ...
  if (!existsSync(dir)) {
    try {
      ...
      throw new StorageError(`Failed to create directory: ${dir}`, 'PERMISSION_ERROR', err);
    }
  }
  ...
  try {
    ...
    throw new StorageError(`Failed to write file: ${filePath}`, 'WRITE_ERROR', err);
  }
}
...
export async function listJsonFiles(dirPath: string): Promise<string[]> {
  if (!existsSync(dirPath)) {
    throw new StorageError(`Directory not found: ${dirPath}`, 'DIRECTORY_NOT_FOUND');
  }
